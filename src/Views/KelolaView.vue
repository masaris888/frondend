    ...
    <ul class="mt-8 space-y-3">
      ...
      >
        ...
        <div class="flex gap-2">
          <button
            @click="editKamar(kamar)"
            class="text-blue-600 hover:text-blue-800 font-medium"
          >
            ✏️ Edit
          </button>
          <button
            @click="hapusKamar(kamar.id)"
  ...
  methods: {
    ...
    tambahKamar() {
      const url = this.editId
        ? `https://kost-backend.alfarizi.workers.dev/kamar/${this.editId}`
        : "https://kost-backend.alfarizi.workers.dev/kamar";
    ...
    hapusKamar(id) {
      fetch(`https://kost-backend.alfarizi.workers.dev/kamar/${id}`, {
        method: "DELETE",
      }).then(() => this.getKamars());
    },
  },
